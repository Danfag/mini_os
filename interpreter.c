...
// Source now abstracts the creation and enqueueing of every process, and returns their pids. Actual execution happens in exec()
int source(char *script) {
    int errCode = 0;
    ...
    int startIndex = disk_current();   //disk_current() returns the next free line in program_lines

    int *pagetable = malloc(sizeof(int) * 33); //pcb page table(33*3= <99 lines per process)
    memset(pagetable, -1, sizeof(int) * 33);
    int currentPage = 0; //page initialisation for  first 2 pages
    int length = 0;
    int page_nb = 0;
    ...
    while (1) {
        ...
        if (feof(p)) {
            if (page_nb < 2) { // clear impertinent data for incomplete pages
                if (i == 1) {
                    memset(page[1], 0, sizeof(line));
                    memset(page[2], 0, sizeof(line));
                    pagetable[currentPage] =
                        program_lines_append(page, pid, currentPage);
                    currentPage++;
                }
                if (i == 2) {
                    memset(page[2], 0, sizeof(line));
        ...
        disk_append(page[i]);//write every line to hard disk
        length++;
        if (page_nb < 2) {
            if (i >= 2) { //upload page to frame store upon completion
                page_nb++;
                pagetable[currentPage] =
                    program_lines_append(page, pid, currentPage);
                currentPage++;
    ...
    fclose(p);
    pcb_init(script_pcb, length, startIndex, pagetable, pid);   // build pcb with computed length and startindex
...
int exec(char *files[], char *policy, int files_nb, int background) {
    ...
    for (int i = 0; i < files_nb; i++) {
        for (int j = 0; j < i; j++) {
            if (strcmp(files[i], files[j]) == 0) { //handling for duplicate processes
                pcb *script_pcb = malloc(sizeof(pcb));
